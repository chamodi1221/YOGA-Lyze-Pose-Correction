...
import cv2
from app.services.dataset_builder import BodyPart, Person, person_from_keypoints_with_scores
import numpy as np
...
class Movement(object):
  """A wrapper class for a Movement TFLite pose estimation model."""
  ...
  def _torso_visible(self, keypoints: np.ndarray) -> bool:
    ...
    Args:
      keypoints: Detection result of Movement model.
    ...
    left_hip_visible = left_hip_score > Movement._MIN_CROP_KEYPOINT_SCORE
    right_hip_visible = right_hip_score > Movement._MIN_CROP_KEYPOINT_SCORE
    left_shoulder_visible = left_shoulder_score > Movement._MIN_CROP_KEYPOINT_SCORE
    right_shoulder_visible = right_shoulder_score > Movement._MIN_CROP_KEYPOINT_SCORE
  ...
  def _determine_torso_and_body_range(self, keypoints: np.ndarray,
    ...
    for idx in range(len(BodyPart)):
      if keypoints[BodyPart(idx).value, 2] < Movement._MIN_CROP_KEYPOINT_SCORE:
        continue
  ...
  def _determine_crop_region(self, keypoints: np.ndarray, image_height: int,
    ...
    if self._torso_visible(keypoints):
      ...
      crop_length_half = np.amax([
          max_torso_xrange * Movement._TORSO_EXPANSION_RATIO,
          max_torso_yrange * Movement._TORSO_EXPANSION_RATIO,
          max_body_yrange * Movement._BODY_EXPANSION_RATIO,
          max_body_xrange * Movement._BODY_EXPANSION_RATIO
      ])
